...
class Servicios {
	...
	function camposTablaViejo($accion,$tabla,$lblcambio,$lblreemplazo,$refdescripcion,$refCampo) {
		...
		switch ($tabla) {
			case 'dbformularios':
				$ocultar = array("leyenda1","leyenda2","leyenda3","refusuarios","refestadocotizacion");
				break;
					...
					if (strpos($row[1],"decimal") !== false) {
						...
						if ( in_array($row[0],$refCampo) ) {
							...
							if (strpos($row[1],"bit") !== false) {
								...
								<div class="col-lg-12 col-md-12 col-sm-12 col-xs-12" style="display:'.$lblOculta.'">
									<label for="'.$campo.'" class="control-label" style="text-align:left">'.$label.'</label>
									<div class="switch">
		...
		switch ($tabla) {
			case 'dbformularios':
				$sqlMod = "select idformulario,
										...
										telefono,
										email,
										(case when aceptacondiciones = 1 then 'Si' else 'No' end) aceptacondiciones,
										(case when opcion2 = 1 then 'Si' else 'No' end) opcion2,
										(case when opcion3 = 1 then 'Si' else 'No' end) opcion3,
										leyenda1,
										leyenda2,
										leyenda3
